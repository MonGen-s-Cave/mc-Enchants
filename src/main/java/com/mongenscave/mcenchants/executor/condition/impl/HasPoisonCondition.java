...
public class HasPoisonCondition extends EnchantCondition {
    @Override
    public boolean evaluate(@NotNull Player player, @NotNull String value, @NotNull Map<String, Object> context) {
        ...
        if (victim instanceof LivingEntity) {
            return ((LivingEntity) victim).hasPotionEffect(PotionEffectType.POISON) == expected;
        }

        Object target = context.get("target");
        if (target instanceof LivingEntity) {
            return ((LivingEntity) target).hasPotionEffect(PotionEffectType.POISON) == expected;
        }

        return player.hasPotionEffect(PotionEffectType.POISON) == expected;
    }
